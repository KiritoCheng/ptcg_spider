    ...
    http
      .get(url, function (res) {
        ...
        res.on("end", function () {
            ...
            .map((i, el) => {
              return {
                type:
                  ...
                  $(el).find("i").attr("class").slice(14).trim(),
                number: $(el).text().trim(),
              };
            })
            .get();
          ...
          let news_item = {
            name:
              cardContent.indexOf("训练师卡") !== -1
                ? $(".card-header").text().trim()
                : $(".pokemon-icon").parent().text().trim(),
            hp: $(".hp").text().trim(),
            attribute: {
...
async function insertDB(url, client, category) {
  for (var i = 1; i < 250; i++) {
    ...
    if (n.hp !== "") {
      client.query("SELECT * FROM " + POKEMON_TABLE, (err, results) => {
        if (err) {
          throw err;
        }
        if (results) {
          ...
          client.query(
            userAddSql,
            {
              ...
              abilitys:
                n.abilitys.length > 0
                  ? n.abilitys.map((k) => JSON.stringify(k)).join(",")
                  : null,
              power:
                n.power.length > 0
                  ? n.power.map((k) => JSON.stringify(k)).join(",")
                  : null,
      ...
      client.query("SELECT * FROM " + TRANIER_TABLE, (err, results) => {
        if (err) {
          throw err;
        }
        if (results) {
...
function start() {
  ...
  pool.getConnection(function (err, client) {
    if (err) {
      ...
      const createTrainerTblSql =
        `CREATE TABLE IF NOT EXISTS ${TRANIER_TABLE} (` +
        `id INT UNSIGNED AUTO_INCREMENT,` +
  ...
  // var client = mysql.createConnection(mysql_config);
  // client.connect();
  // client.on('error', err => {
